...
PhoneBook::PhoneBook() {
  ...
  vector<PhoneEntry> PhoneBook::findByLast(string last) {
    ...
    stmt->execute("SELECT * FROM TestTable WHERE Last like '%"+last+"%'");
...
vector<PhoneEntry> PhoneBook::findByFirst(string first) {
  ...
  vector<PhoneEntry> list;
  stmt->execute("SELECT * FROM TestTable WHERE First like '%"+first+"%'");
  std::auto_ptr< sql::ResultSet > res;
  do {
    res.reset(stmt->getResultSet());
    while (res->next()) {
...
vector<PhoneEntry> PhoneBook::findByType(string type) {
  ...
  vector<PhoneEntry> list;
  stmt->execute("SELECT * FROM TestTable WHERE Type like '%"+type+"%'");
  std::auto_ptr< sql::ResultSet > res;
  do {
    res.reset(stmt->getResultSet());
    while (res->next()) {
...
void PhoneBook::addEntry(string first,string last,string phone, string type){
  ...
  stmt->execute("INSERT INTO TestTable(First,Last,Phone,Type) VALUES ('"+first+"','"+last+"','"+phone+"','"+type+"')");
}
...
void PhoneBook::editEntry(string idnum,string first,string last,string phone, string type){
  ...
  stmt->execute("UPDATE TestTable SET First = '"+first+"', Last ='"+last+"', Phone ='"+phone+"', Type ='"+type+"' WHERE ID='"+idnum+"'");
}
...
void PhoneBook::deleteEntry(string idnum){
  ...
  stmt->execute("DELETE FROM TestTable WHERE ID='"+idnum+"'");
}
